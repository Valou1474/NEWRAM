document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateParallax(scrollY) {
        ...
        const parallaxImages = document.querySelectorAll('[data-parallax], .parallax-image');
        ...
        // Parallax for text blocks (subtle effect)
        const parallaxTexts = document.querySelectorAll('.parallax-element');
        parallaxTexts.forEach(text => {
            const rect = text.getBoundingClientRect();
            const textTop = rect.top + scrollY;
            const windowHeight = window.innerHeight;

            if (rect.top < windowHeight && rect.bottom > 0) {
                const speed = 0.15; // Subtle movement for text
                const yPos = -(scrollY - textTop) * speed;
